...
Line regulation represents the ability of a regulator to withstand static variations from its supply. It is defined as a gain between regulated output and input supply stem:[\Delta V_{out}/ \Delta V_{in}]. The preceding supply might vary due to different causes: its own line/load regulation, bandgap accuracy, static offsets dues to transistor mismatches, temperature drift, etc. 
    

    //check how to implement tabs
Ideally sdsds sdsd
